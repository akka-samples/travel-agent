...
public class TravelPlannerAgent extends Agent {
  ...
  public Effect<TravelPlan> generateTravelPlan(GenerateRequest request) {
    ...
    String userMsg = String.format("""
        ...
        """,
        tripDays, request.destination(),
        request.startDate(), request.endDate(),
        request.budget(), preferencesText);

    return effects()
        .systemMessage(systemMsg)
        .userMessage(userMsg)
        .responseConformsTo(TravelPlan.class)
        .onFailure(throwable -> new TravelPlan(
  ...
  private String formatPreferences(UserProfile profile) {
    ...
    Map<TravelPreference.PreferenceType, List<TravelPreference>> grouped =
        profile.preferences().stream()
